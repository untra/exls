defmodule ExLS.Debugger.Output do
  @moduledoc """
  ...
  def handle_call({:send_response, request_packet, body}, _from, seq) do
    ExLS.IOHandler.send(response(seq, request_packet["seq"], request_packet["command"], body))
    {:reply, :ok, seq + 1}
  end

  def handle_call({:send_event, event, body}, _from, seq) do
    ExLS.IOHandler.send(event(seq, event, body))
    {:reply, :ok, seq + 1}
  end
