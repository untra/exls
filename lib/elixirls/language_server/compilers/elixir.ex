defmodule ExLS.LanguageServer.Compilers.Elixir do
  @moduledoc """
  ...
  @manifest_vsn :exls_v1
  @manifest ".compile.exls"

  alias ExLS.LanguageServer.BuildError
  import Record
  ...
  defp compile_manifest(source_files, manifest, modules, sources, stale, dest, timestamp, opts) do
    ...
    stale_files =
      for path <- stale do
        ...
        case source_file do
          %ExLS.LanguageServer.SourceFile{text: text} -> %{path: path, text: text}
          _ -> %{path: path, text: nil}
        end
      end

    # CHANGED: Capture warnings sent to :standard_error
    alias ExLS.LanguageServer.WarningsTracker
    WarningsTracker.start_link

    try do
      _ = ExLS.LanguageServer.ParallelCompiler.files stale_files,
  ...
  # CHANGED: "source" has changed to "source_file" which is a ExLS.LanguageServer.SourceFile
  ...
  # CHANGED: arg is now a ExLS.LanguageServer.SourceFile struct
