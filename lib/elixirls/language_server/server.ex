defmodule ExLS.LanguageServer.Server do
  @moduledoc """
  ...
  use GenServer
  alias ExLS.LanguageServer.{SourceFile, BuildError, Builder, Protocol, JsonRpc,
  ...
  defp handle_notification(notification("initialized"), state) do
    JsonRpc.show_message(:info, "Initialized with the client! 💎")
    state  # noop
  end

  defp handle_notification(notification("$/setTraceNotification"), state) do
    Logger.debug("XXX TRACE")
    state  # noop
  end
  ...
  defp handle_request(initialize_req(_id, root_uri, client_capabilities), state) do
    JsonRpc.show_message(:warn, "Initializing with the client...")
  ...
  defp handle_request(request(_, _, _), state) do
    Logger.info("SOME OTHER WACKY REQUEST")
  ...
  defp server_capabilities do
    %{"textDocumentSync" => 1,
      ...
      "documentHighlightProvder" => false,
      "documentSymbolProvvider" => false,
      "workspaceSymbolProvider" => false,
      "codeActionProvider" => false,
      "codeLensProvider" => false,
      "ducmentFormattingProvider" => false,
