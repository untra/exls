defmodule ExLS.LanguageServer.ParallelCompiler do
  @moduledoc """
  A fork of Elixir's ParallelCompiler

  This file contains a few changes from Elixir 1.4's ParallelCompiler to make compilation
  work better for use in an IDE. The major changes are:

  1. Don't raise an exception on compilation errors. Instead, we accept an additional option,
     :each_failed_file, which is called with the file and error. This ensures that modules that can
     compile successfully will be compiled, and it also allows us to track all the errors produced
     instead of just the first one.
  2. Files can be passed in either as a path, or as a {path, text} tuple, allowing us to compile
     files that are loaded in memory even if they have changed since they were saved.
  """
  ...
  defp spawn_compilers(%{queued: queued, waiting: waiting, errored: errored, schedulers: schedulers} = state)
      when length(queued) - length(waiting) - length(errored) >= schedulers do
  ...
  defp spawn_compilers(%{entries: [file | files], queued: queued, options: options} = state) do
    parent = self()
    ...
    {pid, ref} =
      :erlang.spawn_monitor fn ->
        ...
        exit(try do
          # CHANGED: Allow files to be {path, text} or {path, nil} instead of just filenames
          # so we don't need to load open files from disk
          case file do
            %{path: path, text: text} when is_binary(path) and is_binary(text) ->
              :elixir_compiler.string(to_charlist(text), path)
            %{path: path, text: nil} when is_binary(path) ->
              :elixir_compiler.file(path)
          end
          {:shutdown, file}
        catch
          kind, reason ->
  ...
  defp wait_for_messages(state) do
    %{entries: entries, options: options, waiting: waiting, queued: queued, errored: errored, result: result} = state
    ...
    receive do
      ...
      {:timed_out, child} ->
        ...
        case List.keyfind(queued, child, 0) do
          # CHANGED: file is now a tuple of {filename, source}
          {^child, _, {file, _}, _} when not is_nil(callback) ->
            callback.(file)
          _ ->
            :ok
        end
      ...
      # CHANGED: Handle success and failure similarly -- invoke callback and continue compiling
      {:DOWN, down_ref, :process, down_pid, reason} ->
        ...
        new_state = %{state | entries: new_entries, waiting: new_waiting, queued: new_queued}
        ...
        case reason do
          {:shutdown, file} ->
            ...
            spawn_compilers(new_state)
          {:failure, _, _, _} ->
            if file = find_failure(down_ref, queued) do
              if callback = Keyword.get(options, :each_failed_file) do
                callback.(file, reason)
              end
            end
            spawn_compilers(%{new_state | errored: [down_pid | errored]})
        end
    end
  end
  ...
  defp print_failure(%ExLS.LanguageServer.SourceFile{path: path}, {:failure, kind, reason, stacktrace}) do
    IO.puts "\n== Compilation error on file #{path} =="
